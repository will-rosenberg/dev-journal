...
Sat/Sun:
  - Continuing to work on my business's app:
    - Working on how to represent possible combinations of phonemes. Realizing there's combinatorial explosion and it can quickly becomes a huge space.
    - Where V represents vowel and C represents consonant, we can represent combinations like V, VC, CV, CVC, CCVC, CVCV, and so on.
    - We likely have somewhere around 18 vowels and 24 consonants. So yeah. Without phonotactic constraints or some way of reducing things, the space quickly becomes very large.
    - Excited about possibilities with this, though. Could somehow represent all possible combinations... block out combinations that English phonotactics don't permit... can then show only what specific words are attested in American English, and with what frequency (could have a waterline / cut-off of some sort, could allow user to select). Could also filter by topic / embedding / something along those lines. Could also do something like what LLMs do (I think) where we predict the next phoneme(s) and/or word(s), based on previous phoneme(s) and/or word(s).
    - Can start with smaller versions of this phoneme space (e.g. just V, VC, CV, CVC), naive ways of navigating/searching/organizing/etc... and also learn how to do this in smarter ways as I go.
    - I've nearly completed a manual / brute force, exhaustive exploration of this space in Google Sheets. It has been valuable to work through this manually.

Summary:
- Shipped major updates to the Accent Explorer and Transcript Viewer, gained strong client feedback, visualized code dependencies, explored phoneme combinatorics, and balanced deep dev work with short-term cashflow needs.

Next/Soon:
- mgAccess-based access control, add tests for Transcript Viewer, expand Accent Explorer’s data logic, and begin translating the phoneme-combination framework into code.
